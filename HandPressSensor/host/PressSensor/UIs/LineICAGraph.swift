...
class LineICAGraph: NSView{
    public var gridColor: NSColor = .black
    public var gridSampleX: Int = 10
    public var gridSampleY: Int = 8
    ...
    public var samples: Int = 8192 // 256本
    ...
    private var ymax: Float = 3.0
    private var ymin: Float = -3.0
    ...
    private func norm(point: Float) -> Float{
        var y: Float = point
        y += abs(self.ymin)
        y /= (abs(self.ymin) + abs(self.ymax))
        return y
    }
    
    public func addPoint(point: Float, index: Int){
        let tmp: Float = norm(point: point)
        let x: Float = clip(point: tmp)
